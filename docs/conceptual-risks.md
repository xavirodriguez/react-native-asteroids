...
| **GC_PRESSURE** | **FIXED** | El crecimiento ilimitado de `trailPositions` en `RenderUpdateSystem` fue resuelto mediante `TrailComponent` y buffer circular. | `RenderUpdateSystem.ts` |
...
| **DETERMINISM** | **LOW**    | El uso de `elapsedTime` en visuales debe ser consistente con la simulación para evitar jitter visual. | `Renderer.ts` |
