...
export abstract class BaseGameStateSystem<TState extends IGameState> extends System {
  ...
  protected gameInstance: BaseGame<Record<string, unknown>, Record<string, unknown>> | undefined;
  protected _world: World | undefined;

  constructor(gameInstance?: BaseGame<Record<string, unknown>, Record<string, unknown>>) {
    super();
  ...
  private handleGameOverStatus(state: TState): void {
    ...
    if (isGameOver) {
      if (!state.gameOverLogged) {
        ...
        if (eventBus) {
          eventBus.emitDeferred("game:over", { gameId: (this.gameInstance as unknown as { gameId: string })?.gameId });
        }
      }
    } else {
