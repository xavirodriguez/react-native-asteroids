"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.FlappyBirdRoom = void 0;
const core_1 = require("@colyseus/core");
const schema_1 = require("@colyseus/schema");
class Bird extends schema_1.Schema {
    x;
    y;
}
__decorate([
    (0, schema_1.type)("number"),
    __metadata("design:type", Number)
], Bird.prototype, "x", void 0);
__decorate([
    (0, schema_1.type)("number"),
    __metadata("design:type", Number)
], Bird.prototype, "y", void 0);
class FlappyState extends schema_1.Schema {
    birds = new schema_1.MapSchema();
}
__decorate([
    (0, schema_1.type)({ map: Bird }),
    __metadata("design:type", Object)
], FlappyState.prototype, "birds", void 0);
class FlappyBirdRoom extends core_1.Room {
    random;
    onCreate(options) {
        this.setState(new FlappyState());
        this.setSimulationInterval((dt) => { });
        this.onMessage("jump", (client) => { });
    }
    onJoin(client) {
        this.state.birds.set(client.sessionId, new Bird());
    }
    onLeave(client) {
        this.state.birds.delete(client.sessionId);
    }
}
exports.FlappyBirdRoom = FlappyBirdRoom;
