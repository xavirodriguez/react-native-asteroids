    ...
    for (let i = 0; i < entities.length; i++) {
      ...
      if (render && render.angularVelocity) {
        ...
        if (!hasOffset) {
          ...
          } as import("../ecs/CoreComponents").VisualOffsetComponent);
          // Component is buffered, so we skip mutation this frame.
          continue;
        }

        world.mutateComponent(entity, "VisualOffset", (v: import("../ecs/CoreComponents").VisualOffsetComponent) => {
