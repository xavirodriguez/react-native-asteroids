...
export class JoystickSystem extends System {
  ...
  public update(world: World, _deltaTime: number): void {
    ...
    for (let i = 0; i < joysticks.length; i++) {
      ...
      world.mutateSingleton<InputStateComponent>("InputState", (state: InputStateComponent) => {
  ...
  private requestHaptic(world: World, pattern: "thrust", intensity: number): void {
    const hapticEntity = world.reserveEntityId();
    world.getCommandBuffer().addCommand(w => {
        w.createEntity(); // This is a bit inconsistent with reserveEntityId if we don't have a way to create with specific ID in CommandBuffer
        // Actually the current WorldCommandBuffer in core doesn't support creating with specific ID.
    });
    // Let's use a simpler approach for now to make it compile
    world.getCommandBuffer().addCommand(w => {
