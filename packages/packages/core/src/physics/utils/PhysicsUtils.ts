...
export class PhysicsUtils {
  ...
  public static integrateMovement(pos: PositionLike, vel: VelocityLike, deltaTimeInSeconds: number): void {
    ...
    const currentX = (pos as Record<string, any>)[xKey] as number;
    const currentY = (pos as Record<string, any>)[yKey] as number;
    const vx = (vel as Record<string, any>)[dxKey] as number;
    const vy = (vel as Record<string, any>)[dyKey] as number;
  ...
  public static applyFriction(vel: VelocityLike, friction: number, deltaTimeMs: number): void {
    const dxKey = vel.dx !== undefined ? "dx" : "velocityX";
    const dyKey = vel.dy !== undefined ? "dy" : "velocityY";

    const dtFactor = deltaTimeMs / (1000 / 60);
    const frictionFactor = Math.pow(friction, dtFactor);
    const v = vel as Record<string, any>;
    if (v[dxKey] !== undefined) v[dxKey] = (v[dxKey] as number) * frictionFactor;
    if (v[dyKey] !== undefined) v[dyKey] = (v[dyKey] as number) * frictionFactor;
  }
